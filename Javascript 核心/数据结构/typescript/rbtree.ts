...
class RBTree implements RBTree.RBTree {
    ...
    add(key: number, data: RBTree.RBTreeNode.data){
        if(this.root === null){
    ...
    addProcess(key: number, data: RBTree.RBTreeNode.data){
        let t: RBTreeNode | null = this.root as RBTreeNode
        let tParentNode:RBTreeNode;
    ...
    // 添加完成后调整
    fixAfterAdd(currentNode: RBTreeNode){
        // 判断从子节点向上遍历调整的依据
        // 当前节点必须是 红色的
        // 父节点是红色的
        // 爷爷节点不能够为空
        ...
        while(
            ...
            if(parentNodeType === 3){
                ...
                // 继续向上遍历调整
                currentNode =  (currentNode.parent && currentNode.parent.parent)as RBTreeNode;
    ...
    getParentNodeType(node: RBTreeNode): number{
        let siblingNodeColor: RBTree.RBTreeNode.color;
    ...
    // 获取插入3节点后的旋转状态
    getNodeType3RotateType(node: RBTreeNode):"LL" | "LR" | "RL" | "RR"{
        let parentNode = node.parent as RBTreeNode
        let grandpaNode = parentNode.parent as RBTreeNode;
 ...
 */
     ...
     * 删除指定的节点
     */
    removeNodeProcess(RBNode: RBTree.RBTreeNode){
        /**
         * 被删除的节点情况分析
    ...
    // 修改后的版本
    fixAfterDel(adjustmentNode: RBTree.RBTreeNode){
        let siblingNode: RBTree.RBTreeNode | null;
        ...
        while(adjustmentNode.parent !== null && adjustmentNode.color === black){
            ...
            if(adjustmentNode === adjustmentNode.parent.left){
                ...
                if(siblingNode && this.getColor(siblingNode.left) === black &&  this.getColor(siblingNode.right) === black){
                    ...
                    this.lRotate(adjustmentNode.parent);
                    adjustmentNode = this.root as RBTree.RBTreeNode  // 停止遍历
     ...
     * @param node 
     */
    getSuccessor(node: RBTree.RBTreeNode): RBTree.RBTreeNode{
        let successor:  RBTree.RBTreeNode = node.right as RBTree.RBTreeNode;
     ...
     * 获取节点的左侧字子节点
     */
    leftOf(node: RBTree.RBTreeNode){
        return node.left;
    }

    // 获取匹配的节点
    getNode(key: number): RBTree.RBTreeNode | null{
        ...
        let currentNode: RBTree.RBTreeNode | null = this.root
        if(currentNode === null){
            return null;
        }
        do{
